...
def plot_ieeg_image(epo, channel, show=False, units="HGP (norm.)", scalings=1, cmap="RdYlBu_r",
    ...
    including custom colormap, confidence intervals, and evoked responses.

    Parameters:
    -----------
    epo : mne.Epochs
        The epochs object containing the data to be plotted.
    channel : str
    ...
    cmap : str or None, optional
        Colormap for the image. Default is "RdYlBu_r". If None, uses default colormap.
    evk_method : str, optional
        Method to compute the evoked response. Default is "mean".
    center : float or None, optional
        Center value for TwoSlopeNorm normalization. Default is 1. If None, no centering is applied.
    ylim_prctile : float or None, optional
        Percentile to define the y-axis limits. Default is 95.
    ci : float, optional
    ...
    if ylim_prctile is None:
        ...
        vmax = np.percentile(data, ylim_prctile)
        vmin = np.percentile(data, 100 - ylim_prctile)
    ...
    # Plot the image:
    figs = mne.viz.plot_epochs_image(epo, picks=channel, show=show,
